...
- The **Backup Tool** download is `VaultPilotBackupTool.exe` version `1.0.1`; **Log Collector** is `VaultPilotLogCollector.exe` version `1.0.1`. Both tools have independent version lines and do not inherit the server version. Legacy PassMan names remain compatibility aliases only.
